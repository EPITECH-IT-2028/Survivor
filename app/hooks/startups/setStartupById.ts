...
interface StartupPayload {
  ...
  sector: string | null;
  project_view: number | null;
  engagement_rate: number | null;
  legacy_id: number | null;
}

export async function setStartupById(
  startupId: number,
  ...
  try {
    ...
    const payload: StartupPayload = {
      ...
      engagement_rate: startupData.engagement_rate,
      project_view: startupData.project_view,
      legacy_id: startupData.legacy_id,
    };
    ...
    if (!res.ok) {
      throw new Error(
        `PUT /api/startups/${startupId} -> ${res.status} ${res.statusText}`,
      );
    }
